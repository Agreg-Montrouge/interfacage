...
from ...connection.daqmx import PyDAQmx
...
class DAQmxAnalogInput(AnalogInput):
    ...
    def start(self, channel_list, sample_rate, block_size, N_block):
        ...
        channel_list = ["{}/ai{}".format(self.device_info._device_str, elm) for elm in channel_list]
        print(channel_list)
        self._channel_list = channel_list
        self._task = AITask(channel_list, sample_rate, block_size)
...
class AITask(Task):
    def __init__(self, channel_list, sample_rate, block_size):
        Task.__init__(self)
        channel_list_str = ','.join(channel_list)
        n_channel = len(channel_list)
        self.CreateAIVoltageChan(channel_list_str, "", PyDAQmx.DAQmx_Val_RSE, -10.0, 10.0, PyDAQmx.DAQmx_Val_Volts, None)
